...
public class EnemyMediumController : MonoBehaviour, IEnemyController
{
    ...
    private void HandleShoot(){
        ...
        if (timeSinceLastShot > timeBetweenShots){ //If the time since the last shot is greater than the time between shots
            Instantiate(projectilePrefab, position: transform.position, rotation: transform.GetChild(0).rotation);
